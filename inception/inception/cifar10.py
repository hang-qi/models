...
def cvx_inference(images, num_class=NUM_CLASSES, for_training=False,
              restore_logits=True, scope=None):
  """A super simple convex linear + softmax model."""
  def linear(inp, out_dim, scope_name='linear'):
    with tf.variable_scope(scope_name) as scope:
      in_dim = inp.get_shape()[1].value
      weights = _variable_with_weight_decay('weights', [in_dim, out_dim],
                                            stddev=1/float(in_dim), wd=0.0)
      biases = _variable_on_cpu('biases', [out_dim], tf.constant_initializer(0.0))
      linear = tf.add(tf.matmul(inp, weights), biases, name=scope.name)
      _activation_summary(linear)
    return linear

  with tf.name_scope(scope, 'simple_linear_softmax', [images]) as sc:
    reshaped = tf.reshape(images, [images.get_shape()[0].value, -1])
    linear1 = linear(reshaped, 512, 'linear1')
    linear2 = linear(linear1, 1024, 'linear2')
    linear3 = linear(linear2, 512, 'linear3')

    # linear layer(WX + b),
    # We don't apply softmax here because
    # tf.nn.sparse_softmax_cross_entropy_with_logits accepts the unscaled logits
    # and performs the softmax internally for efficiency.
    softmax_linear = linear(linear3, NUM_CLASSES, 'softmax_linear')
...
def inference(images, num_classes=NUM_CLASSES, for_training=False,
              restore_logits=True, scope=None):
